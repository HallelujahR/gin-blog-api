...
import (
	...
	"api/internal/modules/content/routes"
	adminRoutes "api/internal/modules/content/routes/admin"
	"api/internal/modules/media"
	"api/internal/platform/logstore"
	"fmt"
	"io"
...
func InitRouter() *gin.Engine {
	...
	if cfg.AccessLogEnabled {
		if writer, path, err := logstore.InitAccessLog(); err != nil {
	...
	// 初始化上传目录
	if err := media.InitUploadDirs(); err != nil {
		// 如果初始化失败，记录错误但不中断启动
