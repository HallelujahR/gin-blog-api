...
func UploadFile(c *gin.Context) {
	...
	// 生成完整访问URL
	fileURL := media.GetFullFileURL(filePath)
...
func StartCompressJob(c *gin.Context) {
	...
	job, err := media.StartCompressJob(files, quality, baseURL)
	if err != nil {
...
func StreamCompressProgress(c *gin.Context) {
	...
	job, ok := media.GetCompressJob(jobID)
	if !ok {
	...
	for {
		...
		case p, ok := <-ch:
			...
			data := media.EncodeProgressEvent(p)
			// SSE 格式: data: <json>\n\n
...
func ListFiles(c *gin.Context) {
	...
	files, total, err := media.ListUploadedFiles(fileType, page, pageSize)
	if err != nil {
	...
	for i := range files {
		if url, ok := files[i]["url"].(string); ok && url != "" {
			files[i]["url"] = media.GetFullFileURL(url)
		}
	}
