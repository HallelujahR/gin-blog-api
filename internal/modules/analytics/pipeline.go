...
var (
	initOnce    sync.Once
	eventCh     chan Event
	rawWriter   *logstore.DailyLogWriter
...
func writeRaw(event Event) error {
	...
	if rawWriter == nil {
		writer, err := logstore.NewDailyLogWriterWithSuffix(rawLogSuffix)
		if err != nil {
			return err
		}
		rawWriter = writer
...
func cacheEvent(event Event) error {
	...
	if event.IP != "" && geoip.IsChinaIP(event.IP) {
		region := geoip.LookupRegion(event.IP)
		if region != "" {
